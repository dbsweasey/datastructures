...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
    ...
    public Entry<K, V> deleteMin() {
        ...
        if (list.size() == 1) {
        	Entry<K, V> toRtn = list.get(0);
        	list.remove(0);
        	return toRtn;
        }
        int bottomRight = list.size() - 1;
        swap(bottomRight, 0);
        Entry<K, V> toRtn = list.get(bottomRight);
        list.removeLast();
        downHeap(0);
        return toRtn;
    }
    ...
    protected void downHeap(int index) {
        ...
        K parent = list.get(index).getKey();
        int child;
        if (!hasRight(index)) {
        	child = left(index);
        } else {
        	child = compare(list.get(left(index)).getKey(), list.get(right(index)).getKey()) < 0 ? left(index) : right(index);
        }
        if (compare(list.get(child).getKey(), parent) < 0) {
        	swap(index, child);
        	downHeap(child);
        }
    }
